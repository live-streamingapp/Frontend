...
export default function CourseManagment11({ order }) {
  ...
  return (
    ...
    <br/>
     <div className="bg-white py-4 pl-2 w-full sm:py-5 sm:pl-[10px]">
      <p className="text-[20px] font-semibold leading-normal sm:text-[24px] font-['SF_Pro']">View & Track Order</p>
    </div>
    <br/>
        ...
        <div className="grid grid-cols-1 gap-4">
          ...
          <div className="space-y-3 text-xs sm:text-sm text-gray-600">
            <div className="flex items-center justify-between">
              ...
              <div className="text-gray-800 text-right font-medium">{o.customerName}</div>
            </div>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <GoGlobe size={12} className="sm:w-4 sm:h-4 flex-shrink-0"/>
              ...
              <div className="text-gray-800 text-right font-medium">{o.product.title}</div>
            </div>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <GoGlobe size={12} className="sm:w-4 sm:h-4 flex-shrink-0"/>
              ...
              <div className="text-gray-800 text-right font-medium">ORD-{o.orderId}</div>
            </div>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <GoGlobe size={12} className="sm:w-4 sm:h-4 flex-shrink-0"/>
              ...
              <div className="text-gray-800 text-right font-medium">{o.orderDate}</div>
            </div>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
               <GoGlobe size={12} className="sm:w-4 sm:h-4 flex-shrink-0"/>
              ...
              <div className="text-gray-800 text-right font-semibold">{o.payableAmount}</div>
            </div>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <GoGlobe size={12} className="sm:w-4 sm:h-4 flex-shrink-0"/>
              ...
              <div className="text-green-600 font-medium text-right">{o.deliveryStatus}</div>
            </div>
          </div>
        </div>
      </div>
   ...
   <div className="bg-white border border-[#E1E1E1] rounded-[15px] shadow-[0_0_15px_0_rgba(0,0,0,0.10)] flex flex-col justify-center items-start gap-[20px] sm:gap-[30px] p-3 sm:p-4">
      ...
      <div className="w-full relative">
        {steps.map((s, idx) => (
          ...
          >
            <FaCircle color="#BB0E00" size={8} className="sm:w-3 sm:h-3 flex-shrink-0" />
            <div className="flex flex-col items-start gap-[1px] sm:gap-[2px]">
              <p className="text-[12px] sm:text-[14px] font-[550] leading-[13px] sm:leading-[15px]">{s.label}</p>
        ...
        {steps.slice(0, -1).map((_, idx) => (
          <div
            ...
            className={`absolute left-[3.5px] sm:left-[-10px] ${
              idx === 0 ? "top-[25px] sm:top-[30px]" : "bottom-[30px] sm:bottom-[40px]"
            }`}
          >
            <PiLineVertical size={24} color="#BB0E00" className="h-[35px] sm:h-[51px] sm:w-8" />
          </div>
        ))}
      </div>
    </div>
        ...
        <div className="flex-1 w-full">
          <div className="flex items-start justify-between gap-2 sm:gap-4">
            <div className="flex-1 text-center sm:text-left">
              <div className="font-semibold text-sm sm:text-base">{o.product.title}</div>
              <div className="text-xs text-gray-500">ORD-{o.orderId}</div>
              <div className="text-xs text-gray-600 mt-1 sm:mt-2">Qty: {o.product.qty}</div>
              ...
              <div className="flex items-center justify-center sm:justify-start gap-1 text-xs text-gray-600 mt-1">
                <img src={SevenMukhiRudraksh} alt="star" className="w-3 h-3 flex-shrink-0" />
                <span className="whitespace-nowrap">{o.product.rating}  ({o.product.reviews} reviews)</span>
              </div>
            </div>
          </div>
        </div>
