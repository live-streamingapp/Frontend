...
const EventsList = () => {
  ...
  const handleDelete = (id) => {
    try {
      ...
      axios.delete(`${import.meta.env.VITE_BACKEND_URL}/events/${id}`);
    } catch (err) {
  ...
  const handleSave = async () => {
    try {
      await axios.put(
        `${import.meta.env.VITE_BACKEND_URL}/events/${selectedEvent._id}`,
        selectedEvent
      );
      ...
      // Refresh events list
      setEvents((prev) =>
        prev.map((ev) => (ev._id === selectedEvent._id ? selectedEvent : ev))
      );
    } catch (err) {
          ...
          <div className="bg-white rounded-xl shadow-lg p-6 w-full max-w-2xl">
            ...
            <div className="space-y-3 max-h-[70vh] overflow-y-auto pr-2">
              ...
              <input
                type="number"
                name="entryAmount"
                value={selectedEvent.entryAmount}
                onChange={handleInputChange}
                className="w-full border rounded p-2 text-sm"
