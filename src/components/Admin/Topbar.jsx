...
const Topbar = ({
  ...
  // Define subpaths where greeting should be hidden
  const hideGreetingOnPaths = ["/astrologers", "/student-management"];

  const shouldHideGreeting = hideGreetingOnPaths.some((path) =>
  ...
  useEffect(() => {
    ...
    if (token) {
      try {
        ...
        // Assuming your token payload has "name" or "username"
        setUser(decoded);
        console.log(decoded);
      } catch (err) {
