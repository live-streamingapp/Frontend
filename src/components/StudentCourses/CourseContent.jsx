...
import { courseVideo } from "../../utils/constants";
...
export default function CourseContent({ crsDetails }) {
  ...
  return (
    <>
      ...
      {courseVideo.length > 6 && (
        <button
          className="mt-3 text-sm underline text-[#BB0E00]"
          onClick={() =>
            setVisibleCount(
              visibleCount === courseVideo.length ? 6 : courseVideo.length
            )
          }
        >
          {visibleCount === courseVideo.length ? "See less" : "See more"}
        </button>
      )}
    </>
  );
